...
[**get_metric**](ArtifactServiceApi.md#get_metric) | **GET** /apis/v2beta1/metrics/{artifact_id} | Gets a metric by task ID and name.
...
# **get_metric**
> V2beta1Artifact get_metric(artifact_id)
...
with kfp_server_api.ApiClient(configuration) as api_client:
    ...
    try:
        # Gets a metric by task ID and name.
        api_response = api_instance.get_metric(artifact_id)
        pprint(api_response)
    except ApiException as e:
...
# **list_metrics**
> V2beta1ListArtifactResponse list_metrics(namespace=namespace, page_token=page_token, page_size=page_size, sort_by=sort_by, filter=filter)
...
filter = 'filter_example' # str | A url-encoded, JSON-serialized filter protocol buffer (see [filter.proto](https://github.com/kubeflow/artifacts/blob/master/backend/api/filter.proto)). (optional)
    ...
    try:
        # Lists all metrics.
        api_response = api_instance.list_metrics(namespace=namespace, page_token=page_token, page_size=page_size, sort_by=sort_by, filter=filter)
        pprint(api_response)
    except ApiException as e:
...
# **log_metric**
> V2beta1Artifact log_metric(body)
