...
class ComparisonUtils:
    ...
    @classmethod
    def compare_pipeline_spec_dicts(cls, actual: dict, expected: dict, **kwargs):
        """
        Compare two pipeline/platform specs
        :param actual: Pipeline Spec that you want to compare
        :param expected: Pipeline Spec that is the source of truth
        :param kwargs: options are: name (pipeline name), display_name (pipeline display name), runtime_params (pipeline runtime params)
        """
        if expected is None:
            ...
            for key, value in expected.items():
                if type(value) == dict:
                    ...
                    cls.compare_pipeline_spec_dicts(actual[key], value, **kwargs)
                else:
