...
class PipelineRunner(BaseRunner):
    ...
    def __init__(self, test_scenario: TestScenario):
        ...
        if self.test_scenario.mode == TestMode.EXPERIMENT:
            ...
            self.experiment_id = self.client.create_experiment(name=experiment_name, description="Experiment to capture performance test pipeline runs", namespace=TestConfig.NAMESPACE).experiment_id
        else:
            experiments: list[V2beta1Experiment] = self.client.list_experiments(page_size=1000).experiments
