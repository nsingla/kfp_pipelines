...
from kfp_server_api import V2beta1Pipeline, V2beta1PipelineVersion, V2beta1Run, V2beta1RuntimeState
...
class PipelineRunner(BaseRunner):
    ...
    def run_pipeline(self, pipeline_id: str) -> V2beta1Run:
        ...
        self.logger.info(f"Creating Pipeline Run for Pipeline with ID: {pipeline_id}, with version ID: {random_pipeline_version}")
        run = self.client.run_pipeline(pipeline_id=pipeline_id, version_id=random_pipeline_version.pipeline_version_id, enable_caching=TestConfig.CACHE_ENABLED)
        self.logger.info(f"Run {run.run_id} created for Pipeline with ID: {pipeline_id}, with version ID: {random_pipeline_version}")
        return run

    async def wait_for_run_to_finish(self, run_id: str):
        pipeline_state = self.client.get_run(run_id).state
        while pipeline_state not in [V2beta1RuntimeState.RUNNING, V2beta1RuntimeState.CANCELING, V2beta1RuntimeState.PENDING]:
    ...
    async def run_pipelines(self, num_of_runs):
        runs_not_completed = [run for run in self.client.list_runs().runs if run.state in [V2beta1RuntimeState.RUNNING, V2beta1RuntimeState.CANCELING, V2beta1RuntimeState.PENDING]]
    ...
    async def stop(self):
        while datetime.datetime.now() < self.test_end_date:
            asyncio.run(self.run_pipelines(self.test_scenario.num_times))
