# KEP-13080: Extensible Multi-Backend Object Storage (Azure) with Unified Storage Layer
...
- an extensible registry model that future backends (e.g. PVC/filesystem, covered in a separate proposal) can plug into.
...
2. PVC/filesystem (`file://`) backend support (covered in a separate proposal due to Kubernetes-specific constraints).
...
- `file://` backend is out of scope for this proposal (see separate PVC/filesystem proposal).
...
Future `azblob://` addition is additive. PVC/filesystem support is addressed in a separate proposal.
